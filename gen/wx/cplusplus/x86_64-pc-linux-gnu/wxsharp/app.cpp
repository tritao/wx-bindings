// ----------------------------------------------------------------------------
// <auto-generated>
// This is autogenerated code by CppSharp.
// Do not edit this file or all your changes will be lost after re-generation.
// </auto-generated>
// ----------------------------------------------------------------------------
#include "wxsharp/app.h"
#include "wxsharp/app.h"
#include "wxsharp/event.h"
#include "wxsharp/object.h"
#include "wxsharp/window.h"

#include <wx/eventfilter.h>

wxSharp::AppConsole::AppConsole(::wxAppConsole* instance, bool ownNativeInstance)
    : wxSharp::EvtHandler((::wxEvtHandler*)instance, ownNativeInstance)
{
    __Instance = instance;

    auto __instance = (wxAppConsole*) __Instance;
    if (__instance && __instance->GetClientData() == nullptr)
        __instance->SetClientData(this);
}

wxSharp::AppConsole::~AppConsole()
{
}

wxSharp::AppConsole::AppConsole()
    : wxSharp::EvtHandler((::wxEvtHandler*)nullptr)
{
    __ownsNativeInstance = true;
    __Instance = new ::wxAppConsole();

    auto __instance = (wxAppConsole*) __Instance;
    if (__instance && __instance->GetClientData() == nullptr)
        __instance->SetClientData(this);
}

bool wxSharp::AppConsole::Initialize(int& argc, wchar_t** argv)
{
    int& __arg0 = (int&)argc;
    auto __arg1 = (::wxChar**)argv;
    bool __ret = ((::wxAppConsole*)__Instance)->Initialize(__arg0, __arg1);
    argc = __arg0;
    return __ret;
}

void wxSharp::AppConsole::CheckSignal()
{
    ((::wxAppConsole*)__Instance)->CheckSignal();
}

bool wxSharp::AppConsole::CallOnInit()
{
    bool __ret = ((::wxAppConsole*)__Instance)->CallOnInit();
    return __ret;
}

bool wxSharp::AppConsole::OnInit()
{
    bool __ret = ((::wxAppConsole*)__Instance)->OnInit();
    return __ret;
}

int wxSharp::AppConsole::OnRun()
{
    int __ret = ((::wxAppConsole*)__Instance)->OnRun();
    return __ret;
}

void wxSharp::AppConsole::OnLaunched()
{
    ((::wxAppConsole*)__Instance)->OnLaunched();
}

int wxSharp::AppConsole::OnExit()
{
    int __ret = ((::wxAppConsole*)__Instance)->OnExit();
    return __ret;
}

void wxSharp::AppConsole::CleanUp()
{
    ((::wxAppConsole*)__Instance)->CleanUp();
}

void wxSharp::AppConsole::OnFatalException()
{
    ((::wxAppConsole*)__Instance)->OnFatalException();
}

void wxSharp::AppConsole::Exit()
{
    ((::wxAppConsole*)__Instance)->Exit();
}

const char* wxSharp::AppConsole::GetAppName() const
{
    ::wxString __ret = ((::wxAppConsole*)__Instance)->GetAppName();
    return __ret.c_str();
}

void wxSharp::AppConsole::SetAppName(const char* name)
{
    auto __arg0 = name;
    ((::wxAppConsole*)__Instance)->SetAppName(__arg0);
}

const char* wxSharp::AppConsole::GetAppDisplayName() const
{
    ::wxString __ret = ((::wxAppConsole*)__Instance)->GetAppDisplayName();
    return __ret.c_str();
}

void wxSharp::AppConsole::SetAppDisplayName(const char* name)
{
    auto __arg0 = name;
    ((::wxAppConsole*)__Instance)->SetAppDisplayName(__arg0);
}

const char* wxSharp::AppConsole::GetClassName() const
{
    ::wxString __ret = ((::wxAppConsole*)__Instance)->GetClassName();
    return __ret.c_str();
}

void wxSharp::AppConsole::SetClassName(const char* name)
{
    auto __arg0 = name;
    ((::wxAppConsole*)__Instance)->SetClassName(__arg0);
}

void wxSharp::AppConsole::SetVendorName(const char* name)
{
    auto __arg0 = name;
    ((::wxAppConsole*)__Instance)->SetVendorName(__arg0);
}

void wxSharp::AppConsole::SetVendorDisplayName(const char* name)
{
    auto __arg0 = name;
    ((::wxAppConsole*)__Instance)->SetVendorDisplayName(__arg0);
}

void wxSharp::AppConsole::SetCLocale()
{
    ((::wxAppConsole*)__Instance)->SetCLocale();
}

bool wxSharp::AppConsole::IsMainLoopRunning()
{
    bool __ret = ::wxAppConsoleBase::IsMainLoopRunning();
    return __ret;
}

void wxSharp::AppConsole::OnUnhandledException()
{
    ((::wxAppConsole*)__Instance)->OnUnhandledException();
}

bool wxSharp::AppConsole::OnExceptionInMainLoop()
{
    bool __ret = ((::wxAppConsole*)__Instance)->OnExceptionInMainLoop();
    return __ret;
}

bool wxSharp::AppConsole::StoreCurrentException()
{
    bool __ret = ((::wxAppConsole*)__Instance)->StoreCurrentException();
    return __ret;
}

void wxSharp::AppConsole::RethrowStoredException()
{
    ((::wxAppConsole*)__Instance)->RethrowStoredException();
}

void wxSharp::AppConsole::ProcessPendingEvents()
{
    ((::wxAppConsole*)__Instance)->ProcessPendingEvents();
}

bool wxSharp::AppConsole::HasPendingEvents() const
{
    bool __ret = ((::wxAppConsole*)__Instance)->HasPendingEvents();
    return __ret;
}

void wxSharp::AppConsole::SuspendProcessingOfPendingEvents()
{
    ((::wxAppConsole*)__Instance)->SuspendProcessingOfPendingEvents();
}

void wxSharp::AppConsole::ResumeProcessingOfPendingEvents()
{
    ((::wxAppConsole*)__Instance)->ResumeProcessingOfPendingEvents();
}

void wxSharp::AppConsole::RemovePendingEventHandler(::wxSharp::EvtHandler* toRemove)
{
    auto __arg0 = toRemove ? (::wxEvtHandler*)toRemove->__Instance : nullptr;
    ((::wxAppConsole*)__Instance)->RemovePendingEventHandler(__arg0);
}

void wxSharp::AppConsole::AppendPendingEventHandler(::wxSharp::EvtHandler* toAppend)
{
    auto __arg0 = toAppend ? (::wxEvtHandler*)toAppend->__Instance : nullptr;
    ((::wxAppConsole*)__Instance)->AppendPendingEventHandler(__arg0);
}

void wxSharp::AppConsole::DelayPendingEventHandler(::wxSharp::EvtHandler* toDelay)
{
    auto __arg0 = toDelay ? (::wxEvtHandler*)toDelay->__Instance : nullptr;
    ((::wxAppConsole*)__Instance)->DelayPendingEventHandler(__arg0);
}

void wxSharp::AppConsole::DeletePendingEvents()
{
    ((::wxAppConsole*)__Instance)->DeletePendingEvents();
}

void wxSharp::AppConsole::ScheduleForDestruction(::wxSharp::Object* object)
{
    auto __arg0 = object ? (::wxObject*)object->__Instance : nullptr;
    ((::wxAppConsole*)__Instance)->ScheduleForDestruction(__arg0);
}

bool wxSharp::AppConsole::IsScheduledForDestruction(::wxSharp::Object* object) const
{
    auto __arg0 = object ? (::wxObject*)object->__Instance : nullptr;
    bool __ret = ((::wxAppConsole*)__Instance)->IsScheduledForDestruction(__arg0);
    return __ret;
}

bool wxSharp::AppConsole::Pending()
{
    bool __ret = ((::wxAppConsole*)__Instance)->Pending();
    return __ret;
}

bool wxSharp::AppConsole::Dispatch()
{
    bool __ret = ((::wxAppConsole*)__Instance)->Dispatch();
    return __ret;
}

int wxSharp::AppConsole::MainLoop()
{
    int __ret = ((::wxAppConsole*)__Instance)->MainLoop();
    return __ret;
}

void wxSharp::AppConsole::ExitMainLoop()
{
    ((::wxAppConsole*)__Instance)->ExitMainLoop();
}

bool wxSharp::AppConsole::Yield(bool onlyIfNeeded)
{
    bool __ret = ((::wxAppConsole*)__Instance)->Yield(onlyIfNeeded);
    return __ret;
}

void wxSharp::AppConsole::WakeUpIdle()
{
    ((::wxAppConsole*)__Instance)->WakeUpIdle();
}

bool wxSharp::AppConsole::ProcessIdle()
{
    bool __ret = ((::wxAppConsole*)__Instance)->ProcessIdle();
    return __ret;
}

bool wxSharp::AppConsole::UsesEventLoop() const
{
    bool __ret = ((::wxAppConsole*)__Instance)->UsesEventLoop();
    return __ret;
}

void wxSharp::AppConsole::OnAssertFailure(wchar_t* file, int line, wchar_t* func, wchar_t* cond, wchar_t* msg)
{
    auto __arg0 = (::wxChar*)file;
    auto __arg2 = (::wxChar*)func;
    auto __arg3 = (::wxChar*)cond;
    auto __arg4 = (::wxChar*)msg;
    ((::wxAppConsole*)__Instance)->OnAssertFailure(__arg0, line, __arg2, __arg3, __arg4);
}

void wxSharp::AppConsole::OnAssert(wchar_t* file, int line, wchar_t* cond, wchar_t* msg)
{
    auto __arg0 = (::wxChar*)file;
    auto __arg2 = (::wxChar*)cond;
    auto __arg3 = (::wxChar*)msg;
    ((::wxAppConsole*)__Instance)->OnAssert(__arg0, line, __arg2, __arg3);
}

bool wxSharp::AppConsole::CheckBuildOptions(const char* optionsSignature, const char* componentName)
{
    auto __arg0 = (const char*)optionsSignature;
    auto __arg1 = (const char*)componentName;
    bool __ret = ::wxAppConsoleBase::CheckBuildOptions(__arg0, __arg1);
    return __ret;
}

::wxSharp::AppConsole* wxSharp::AppConsole::GetInstance()
{
    ::wxAppConsole* __ret = ::wxAppConsoleBase::GetInstance();
    return (__ret == nullptr) ? nullptr : new wxSharp::AppConsole((::wxAppConsole*)__ret);
}

void wxSharp::AppConsole::SetInstance(::wxSharp::AppConsole* app)
{
    auto __arg0 = app ? (::wxAppConsole*)app->__Instance : nullptr;
    ::wxAppConsoleBase::SetInstance(__arg0);
}

int wxSharp::AppConsole::get_argc()
{
    int __ret = ((::wxAppConsole*)__Instance)->argc;
    return __ret;
}


void wxSharp::AppConsole::set_argc(int value)
{
    ((::wxAppConsole*)__Instance)->argc = value;
}

wxSharp::App::App(::wxApp* instance, bool ownNativeInstance)
    : wxSharp::AppConsole((::wxAppConsole*)instance, ownNativeInstance)
{
    __Instance = instance;

    auto __instance = (wxApp*) __Instance;
    if (__instance && __instance->GetClientData() == nullptr)
        __instance->SetClientData(this);
}

wxSharp::App::~App()
{
}

wxSharp::App::App()
    : wxSharp::AppConsole((::wxAppConsole*)nullptr)
{
    __ownsNativeInstance = true;
    __Instance = new ::wxApp();

    auto __instance = (wxApp*) __Instance;
    if (__instance && __instance->GetClientData() == nullptr)
        __instance->SetClientData(this);
}

bool wxSharp::App::SetNativeTheme(const char* theme)
{
    auto __arg0 = theme;
    bool __ret = ((::wxApp*)__Instance)->SetNativeTheme(__arg0);
    return __ret;
}

bool wxSharp::App::OnInitGui()
{
    bool __ret = ((::wxApp*)__Instance)->OnInitGui();
    return __ret;
}

void wxSharp::App::WakeUpIdle()
{
    ((::wxApp*)__Instance)->WakeUpIdle();
}

bool wxSharp::App::Initialize(int& argc, wchar_t** argv)
{
    int& __arg0 = (int&)argc;
    auto __arg1 = (::wxChar**)argv;
    bool __ret = ((::wxApp*)__Instance)->Initialize(__arg0, __arg1);
    argc = __arg0;
    return __ret;
}

void wxSharp::App::CleanUp()
{
    ((::wxApp*)__Instance)->CleanUp();
}

void wxSharp::App::OnAssertFailure(wchar_t* file, int line, wchar_t* func, wchar_t* cond, wchar_t* msg)
{
    auto __arg0 = (::wxChar*)file;
    auto __arg2 = (::wxChar*)func;
    auto __arg3 = (::wxChar*)cond;
    auto __arg4 = (::wxChar*)msg;
    ((::wxApp*)__Instance)->OnAssertFailure(__arg0, line, __arg2, __arg3, __arg4);
}

void* wxSharp::App::GetXVisualInfo()
{
    void* __ret = ((::wxApp*)__Instance)->GetXVisualInfo();
    return __ret;
}

bool wxSharp::App::EventsPending()
{
    bool __ret = ((::wxApp*)__Instance)->EventsPending();
    return __ret;
}

bool wxSharp::App::DoIdle()
{
    bool __ret = ((::wxApp*)__Instance)->DoIdle();
    return __ret;
}

int wxSharp::App::OnRun()
{
    int __ret = ((::wxApp*)__Instance)->OnRun();
    return __ret;
}

int wxSharp::App::OnExit()
{
    int __ret = ((::wxApp*)__Instance)->OnExit();
    return __ret;
}

bool wxSharp::App::SafeYield(::wxSharp::Window* win, bool onlyIfNeeded)
{
    auto __arg0 = win ? (::wxWindow*)win->__Instance : nullptr;
    bool __ret = ((::wxApp*)__Instance)->SafeYield(__arg0, onlyIfNeeded);
    return __ret;
}

bool wxSharp::App::SafeYieldFor(::wxSharp::Window* win, long eventsToProcess)
{
    auto __arg0 = win ? (::wxWindow*)win->__Instance : nullptr;
    bool __ret = ((::wxApp*)__Instance)->SafeYieldFor(__arg0, eventsToProcess);
    return __ret;
}

bool wxSharp::App::ProcessIdle()
{
    bool __ret = ((::wxApp*)__Instance)->ProcessIdle();
    return __ret;
}

bool wxSharp::App::UsesEventLoop() const
{
    bool __ret = ((::wxApp*)__Instance)->UsesEventLoop();
    return __ret;
}

bool wxSharp::App::IsActive() const
{
    bool __ret = ((::wxApp*)__Instance)->IsActive();
    return __ret;
}

void wxSharp::App::SetTopWindow(::wxSharp::Window* win)
{
    auto __arg0 = win ? (::wxWindow*)win->__Instance : nullptr;
    ((::wxApp*)__Instance)->SetTopWindow(__arg0);
}

::wxSharp::Window* wxSharp::App::GetTopWindow() const
{
    ::wxWindow* __ret = ((::wxApp*)__Instance)->GetTopWindow();
    return (__ret == nullptr) ? nullptr : new wxSharp::Window((::wxWindow*)__ret);
}

void wxSharp::App::SetExitOnFrameDelete(bool flag)
{
    ((::wxApp*)__Instance)->SetExitOnFrameDelete(flag);
}

bool wxSharp::App::GetExitOnFrameDelete() const
{
    bool __ret = ((::wxApp*)__Instance)->GetExitOnFrameDelete();
    return __ret;
}

void wxSharp::App::SetUseBestVisual(bool flag, bool forceTrueColour)
{
    ((::wxApp*)__Instance)->SetUseBestVisual(flag, forceTrueColour);
}

bool wxSharp::App::GetUseBestVisual() const
{
    bool __ret = ((::wxApp*)__Instance)->GetUseBestVisual();
    return __ret;
}

void wxSharp::App::SetPrintMode(int _0)
{
    ((::wxApp*)__Instance)->SetPrintMode(_0);
}

int wxSharp::App::GetPrintMode() const
{
    int __ret = ((::wxApp*)__Instance)->GetPrintMode();
    return __ret;
}

void wxSharp::App::SetActive(bool isActive, ::wxSharp::Window* lastFocus)
{
    auto __arg1 = lastFocus ? (::wxWindow*)lastFocus->__Instance : nullptr;
    ((::wxApp*)__Instance)->SetActive(isActive, __arg1);
}

int wxSharp::App::HandleEvent(::wxEvent& event)
{
    wxEventType eventType = event.GetEventType();

    if(eventType == wxEVT_QUERY_END_SESSION)
    {
        if (!OnQueryEndSession.empty())
        {
            wxCloseEvent* _event = wxStaticCast(&event, wxCloseEvent);
            wxSharp::CloseEvent _e(_event);
            OnQueryEndSession(_e);
        }
    }
    else if(eventType == wxEVT_END_SESSION)
    {
        if (!OnEndSession.empty())
        {
            wxCloseEvent* _event = wxStaticCast(&event, wxCloseEvent);
            wxSharp::CloseEvent _e(_event);
            OnEndSession(_e);
        }
    }
    else if(eventType == wxEVT_ACTIVATE_APP)
    {
        if (!OnActivateApp.empty())
        {
            wxActivateEvent* _event = wxStaticCast(&event, wxActivateEvent);
            wxSharp::ActivateEvent _e(_event);
            OnActivateApp(_e);
        }
    }
    else if(eventType == wxEVT_HIBERNATE)
    {
        if (!OnHibernate.empty())
        {
            wxActivateEvent* _event = wxStaticCast(&event, wxActivateEvent);
            wxSharp::ActivateEvent _e(_event);
            OnHibernate(_e);
        }
    }
    else if(eventType == wxEVT_IDLE)
    {
        if (!OnIdle.empty())
        {
            wxIdleEvent* _event = wxStaticCast(&event, wxIdleEvent);
            wxSharp::IdleEvent _e(_event);
            OnIdle(_e);
        }
    }

    return AppConsole::HandleEvent(event);
}

void (::wxSharp::Exit)()
{
    ::wxExit();
}

void (::wxSharp::WakeUpIdle)()
{
    ::wxWakeUpIdle();
}

::wxSharp::AppConsole* (::wxSharp::CreateApp)()
{
    ::wxAppConsole* __ret = ::wxCreateApp();
    return (__ret == nullptr) ? nullptr : new wxSharp::AppConsole((::wxAppConsole*)__ret);
}
