    ...
    int argc, JSValueConst* argv)
{
    ...
    JSValue ____ret = JS_Interop_CreateFromInstance(ctx, classId_Ozone_FontMetrics, JS_INTEROP_INSTANCE_RAW_POINTER, (void*) &__ret);
...
typecheck15:
    if (JS_IsInt32(argv[9]))
        goto typecheck16;

    goto error;

typecheck16:
    if (JS_IsInt32(argv[10]))
        goto overload0;
...
typecheck18:
    if (JS_IsInt32(argv[11]))
        goto typecheck19;

    goto error;

typecheck19:
    if (JS_IsInt32(argv[12]))
        goto overload0;
